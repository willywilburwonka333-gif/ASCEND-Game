...
function showIncubator(){
 ...
 const panel=modalParts().panel;
 panel.querySelectorAll('[data-v82-egg]').forEach(button=>button.onclick=()=>paintEgg(Number(button.dataset.v82Egg)));
 panel.querySelector('[data-v82-close]').onclick=closeOwn;
}
...
function paintEgg(index){
 ...
 panel.querySelector('[data-small]').onclick=()=>size=8;panel.querySelector('[data-medium]').onclick=()=>size=16;panel.querySelector('[data-large]').onclick=()=>size=28;panel.querySelector('[data-eraser]').onclick=()=>erasing=true;panel.querySelector('[data-clear]').onclick=shell;panel.querySelector('[data-back]').onclick=showIncubator;
 panel.querySelector('[data-save]').onclick=()=>{const art=load(ART_KEY);art[egg.id]={name:egg.name,image:canvas.toDataURL('image/png'),updatedAt:Date.now()};save(ART_KEY,art);hatchQuestion(index,0,0)};
}
...
function hatchQuestion(index,questionIndex,energy){
 ...
 const panel=modalParts().panel;
 panel.querySelectorAll('[data-v82-answer]').forEach(button=>button.onclick=()=>{if(button.dataset.v82Answer!==question.a){panel.querySelector('.fb').textContent='Not yet. Use the hint and try again.';return}const next=Math.min(100,energy+25);button.disabled=true;setTimeout(()=>next>=100?finishHatch(index):hatchQuestion(index,questionIndex+1,next),180)});
 panel.querySelector('[data-hint]').onclick=()=>panel.querySelector('.fb').textContent=question.h;
}
...
function finishHatch(index){
 ...
 const panel=modalParts().panel;panel.querySelector('[data-another]').onclick=s.eggs.length?showIncubator:closeOwn;panel.querySelector('[data-v82-close]').onclick=closeOwn;
}
...
function enhancePhysicalEntrance(){
 ...
 const old=panel.querySelector('[data-v4paint]');
 if(old){old.textContent='🎨 PAINT & HATCH EGGS';old.removeAttribute('data-v4paint');old.dataset.v82Open='1'}
 if(!panel.querySelector('[data-v82-open]')){const button=document.createElement('button');button.dataset.v82Open='1';button.textContent='🥚 OPEN INCUBATOR';button.style.margin='8px 0';panel.querySelector('.v81-card')?.after(button)}
 enhancing=false;
}

document.addEventListener('click',event=>{const button=event.target.closest?.('[data-v82-open]');if(!button)return;event.preventDefault();event.stopPropagation();showIncubator()},true);
...
const badge=document.getElementById('build-badge');if(badge)badge.textContent='VERDARA V8.3 · INCUBATOR FIX';